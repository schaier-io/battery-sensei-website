 ...
 * unit-testable: id sanitizing, card-type parsing, and a download href that
 * does not pass referral identifiers to the third-party file host.
 */
...
/**
 * Keep attribution on the first-party landing request. The download route
 * redirects to GitHub, so forwarding `ref` or campaign values would disclose
 * them to GitHub without helping the app download itself.
 */
export function downloadHref(_search: ReferralSearch): string {
